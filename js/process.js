...
const kbMode = Object.freeze({ "typing": 1, "word_selection": 2, "clear": 3 });
var curKbMode = kbMode.typing;

function changeState(newState) {
    // change the state and draw the new keyboard
    curKbMode = newState;
        ...
        case kbMode.typing:
            if (curr_button != 5 && curr_button >= 0) {
                ...
                if (typeof final_words == 'undefined' ||
                    final_words == '') {
                    changeState(kbMode.clear);
                } else {
                    // we have a list of words already. Set the mode to the word selection
                    changeState(kbMode.word_selection);
                }
            }
            break;
        case kbMode.word_selection:
...
function onResults(results) {
    ...
    if (results.detections.length > 0) {
        drawRectangle(
            canvasCtx, results.detections[0].boundingBox, { color: 'blue', lineWidth: 4, fillColor: '#00000000' });
